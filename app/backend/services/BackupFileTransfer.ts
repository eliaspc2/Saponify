...
export class BackupFileTransfer {
    public downloadBackup(json: string) {
        ...
        const filename = `${AppConstants.BACKUP_FILE_PREFIX}${timestamp}${AppConstants.BACKUP_FILE_EXTENSION}`;
    ...
    public downloadAutoBackup(data: string, timestamp: string) {
        ...
        const filename = `${AppConstants.AUTO_BACKUP_FILE_PREFIX}${safeTimestamp}${AppConstants.BACKUP_FILE_EXTENSION}`;
